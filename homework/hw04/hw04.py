...
def balanced(m):
    ...
    layer = (total_weight(left_arm)*length(left(m))) == (total_weight(right_arm)*length(right(m)))
    return le and ri and layer
...
def make_withdraw(balance, password):
    ...
    password_list = []
    def account(d_money, password_input):
        nonlocal balance
...
def make_joint(withdraw, old_pass, new_pass):
    ...
    def account(money_d, password_input):
        nonlocal withdraw
        if password_input == new_pass or password_input == old_pass:
            return withdraw(money_d, old_pass)
        else:
            return withdraw(money_d, password_input)
    return account
...
def multiple_references_explanation():
    return """Multiple references of an unknown variable cause the multiple reference issue. 
Error boundaries grow with repetition."""
...
def quadratic(x, a, b, c):
    ...
    Xtrm_x = -b/(2*a)
    y1, y2, Xtrm_y = [f(x) for x in [lower_bound(x), upper_bound(x), Xtrm_x]]
    if lower_bound(x) < Xtrm_x < upper_bound(x):
        return interval(min(y1, y2, Xtrm_y), max(y1, y2, Xtrm_y))
    else:
