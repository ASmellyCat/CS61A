...
def distance(city1, city2):
    ...
    """
    "*** YOUR CODE HERE ***"
    dellon = (get_lon(city1) - get_lon(city2)) ** 2
    dellat = (get_lat(city1) - get_lat(city2)) ** 2
    return sqrt(dellon + dellat)
...
def add_trees(t1, t2):
    ...
    if is_leaf(t1) or is_leaf(t2):
        ...
        bran = [add_trees(i, j) for i, j in zip(bran1, bran2)]
        if len(bran1) > len(bran2):
            bran += bran1[len(bran2):]
        else:
            bran += bran2[len(bran1):]
    return tree(labels, bran)
